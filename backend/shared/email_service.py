...
class EmailService:
    def __init__(self, logger):
        ...
        self.smtp_password = self._resolve_smtp_password()

        # SES Config (lazy init)
        self.ses_client = None

    def _resolve_smtp_password(self):
        """Prefer SSM SecureString; fall back to legacy SMTP_PASSWORD env var."""
        ssm_param = os.environ.get('SMTP_PASSWORD_SSM_PARAM')
        if ssm_param:
            try:
                ...
                self.logger.error(f"Failed to load SMTP password from SSM ({ssm_param}): {exc}")
                return None
    ...
    def _send_ses(self, to_email, subject, body):
        ...
        try:
            self.ses_client.send_email(
                Source=self.smtp_user or "noreply@example.com",
