...
resource "aws_apigatewayv2_api" "http_api" {
  ...
  cors_configuration {
    allow_origins = ["https://${var.cloudfront_domain_name}", "http://localhost:5500", "http://127.0.0.1:5500"]
...
resource "aws_apigatewayv2_authorizer" "admin_auth" {
  ...
  authorizer_uri                    = var.lambda_invoke_arns["authorizer"]
  identity_sources                  = ["$request.header.x-api-key"]
  name                              = "${var.prefix}-admin-authorizer"
